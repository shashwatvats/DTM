...
$columns = array('faculty_task_id','hod_task_name','hod_task_description','hod_task_type','hod_task_priority', 'hod_task_assign_on','hod_task_deadline','faculty_task_status','faculty_task_date_of_completed','faculty_task_assign_by');
...
if(isset($_POST["search"]["value"]))
{
 ...
 OR faculty_task_date_of_completed LIKE "%'.vi($_POST["search"]["value"]).'%"
 OR faculty_task_assign_by LIKE "%'.vi($_POST["search"]["value"]).'%"
 )
 ';
}
...
while($row = mysqli_fetch_array($result))
{
 ...
 if($row["faculty_task_status"] == 100){
  ...
  $sub_array[] = '<div id="'.$row["faculty_task_id"].'" data-column="faculty_task_date_of_completed">' . date("h:i:s A, d/M/Y",strtotime($row["faculty_task_date_of_completed"])) . '</div>';
  
  $sub_array[] = '<div id="'.$row["faculty_task_id"].'" data-column="faculty_task_assign_by">' . $row["faculty_task_assign_by"] . '</div>';
 ...
 //$sub_array[] = '<button type="button" name="faculty_task_delete" class="btn btn-danger btn-xs faculty_task_delete" id="'.$row["faculty_task_id"].'">Delete</button>';
...
$faculty_department = $_SESSION["faculty_department"]; //outside variable do not work inside the functions
 $query = "SELECT * FROM faculty_".$faculty_department."_".$faculty_username."";
 $result = mysqli_query($connect, $query);
